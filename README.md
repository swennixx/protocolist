...
- **Анализ в два прохода**: обзор всей встречи и поиск решений/задач окнами по 40 реплик — маленькой модели проще быть внимательной на коротком фрагменте.
- **Map-reduce для длинных встреч**: стенограмма длиннее ~18 тыс. символов (≈ 20 минут разговора) режется на части; из каждой модель выписывает факты, темы и имена, затем одна свёртка собирает выжимку и 3–8 тем всей встречи. Если заметок слишком много (многочасовые встречи), свёртка идёт уровнями — каждый уровень минимум вдвое сокращает число частей. На 15-минутном подкасте, разбитом на 4 части, выжимка сохранила цифры и даты, которые обзор одним запросом опустил; цена — время: 140 с против 53 с.
